    ...
    public class BoardState
    {
        ...
        private int CheckForDiagonalWin()
        {
            ...
            if (Board[0, Board.GetLength(1) - 1] != 0)
            {
                ...
                for (int i = 1; i < Board.GetLength(0); i++)
                {
                    if (Board[i, Board.GetLength(1) - 1 - i] == 0)
                        break;
                    if (playerSymbol != Board[i, Board.GetLength(1) - 1 - i])
                        break;
                    playerTilesInDiagonal++;
                }
        ...
        public float CheckForPrematureScore(bool isMaximizing)
        {
            ...
            {
                return Math.Max(
                    Math.Max(horizontalPrematureScore, verticalPrematureScore),
                    diagonalPrematureScore
                );
            }
            else
            {
                return Math.Min(
                    Math.Min(horizontalPrematureScore, verticalPrematureScore),
                    diagonalPrematureScore
                );
            }
        }
        ...
        private float CheckForHorizontalPrematureScore()
        {
            ...
            {
                if (
                    playersWithAlmostTerminalRows.Exists(player =>
        ...
        private float CheckForVerticalPrematureScore()
        {
            ...
            {
                if (
                    playersWithAlmostTerminalCols.Exists(player =>
                        CurrentPlayer == _playingField.CurrentPlayer
                        && player != _playingField.CurrentPlayer
                    )
                )
        ...
        private float CheckForDiagonalPrematureScore()
        {
...

            {
                if (!_alreadyNotPossibleDiagonals.Contains(1))
                {
                    ...
                    diagonalScoresForPlayers[playerSymbol] +=
                        (float)playerTilesInDiagonal / Board.GetLength(0);
            ...
            {
                if (
                    playersWithAlmostTerminalDiagonals.Exists(player =>
                        CurrentPlayer == _playingField.CurrentPlayer
                        && player != _playingField.CurrentPlayer
                    )
                )
